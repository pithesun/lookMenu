...
public class MenuActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 특정 음식 리뷰
        fc.getRecommended_reviews().observe(this, recommends -> {
            for( Review review : recommends) {
                Log.d("MenuActivity", review.getReview());
            }
        });

        // 전체 리뷰
        fc.getReviewForUser().observe(this, recommends -> {
            for( Review review : recommends) {
                Log.d("MenuActivity", review.getReview());
            }
        });
